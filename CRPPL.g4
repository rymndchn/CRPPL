...
altercolumn: (NEWCOLUMN|DELETECOLUMN) (LITERAL|IDENTIFIER) FOR (LITERAL|IDENTIFIER);
...
createfunction: CREATEFUNCTION IDENTIFIER OPENPARENTHESIS (IDENTIFIER (SEPARATOR IDENTIFIER)*)? CLOSEPARENTHESIS ((generalquery | importfile | altercolumn | defineconstant | functioncall | graphquery | conditionalstatement)*?) (RETURN | (RETURN IDENTIFIER))? ENDFUNCTION;
...
booleanstatement: (
  ...
  OPENPARENTHESIS (NEGATOR)?(LITERAL|IDENTIFIER|TRUE|FALSE| booleanstatement) (OPERATOR (NEGATOR)?(LITERAL|IDENTIFIER|TRUE|FALSE|booleanstatement))* CLOSEPARENTHESIS
);
...
OPERATOR: ( EQUALS | GREATERTHAN | GREATERTHANEQUAL | LESSTHAN | LESSTHANEQUAL | NOTEQUAL | AND | OR | XOR);
NEGATOR: NOT;
